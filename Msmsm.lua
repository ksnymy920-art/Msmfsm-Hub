...
local Config = {
    ...
    OptimizerEnabled = false, EspEnabled = false,
    FovEnabled = false, AntiFlingEnabled = false,
    BrainrotDefenseEnabled = false, BrainrotDefenseSide = "left",
...
Screen.Name = "MsmsmHub_v8"; Screen.ResetOnSpawn = false
...
local function notify(msg, dur)
    ...
    Instance.new("UIStroke", f).Color = Color3.fromRGB(136, 0, 255)
    ...
    task.delay(dur or 2.5, function()
        TweenService:Create(f, TweenInfo.new(0.3), {Position = UDim2.new(0.5, -110, 0, -50)}):Play()
        task.wait(0.3)
        g:Destroy()
    end)
end
...
-- ====================== DISCORD BUTTON ======================
local DL = "https://discord.gg/XpxhSsYfUc"
local function copyDiscord()
    pcall(function()
        setclipboard(DL)
        notify("✅ Discord Copied!", 2)
    end)
end

local DF = Instance.new("Frame", Screen)
DF.Size = UDim2.new(0, 160, 0, 32)
DF.Position = UDim2.new(1, -170, 0, 5)
DF.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
DF.BorderSizePixel = 0
DF.ZIndex = 200
Instance.new("UICorner", DF).CornerRadius = UDim.new(0, 8)
local dS = Instance.new("UIStroke", DF)
dS.Color = Color3.fromRGB(136, 0, 255)
dS.Thickness = 1.5
local dL = Instance.new("TextLabel", DF)
dL.Size = UDim2.new(1, -28, 1, 0)
dL.Position = UDim2.new(0, 8, 0, 0)
...
dL.ZIndex = 201
local cI = Instance.new("TextButton", DF)
cI.Size = UDim2.new(0, 24, 0, 24)
cI.Position = UDim2.new(1, -28, 0.5, -12)
...
cI.TextSize = 12
cI.ZIndex = 202
cI.MouseButton1Click:Connect(copyDiscord)
task.spawn(function()
    local t = 0
    while DF and DF.Parent do
        t = t + 0.03
        dS.Color = Color3.fromRGB(math.clamp(136 + math.sin(t * 3) * 60, 0, 255), 0, math.clamp(255 - math.sin(t * 3) * 60, 0, 255))
        task.wait(0.03)
    end
end)
local dB = Instance.new("TextButton", DF)
dB.Size = UDim2.new(1, -30, 1, 0)
dB.BackgroundTransparency = 1
dB.Text = ""
dB.ZIndex = 203
dB.MouseButton1Click:Connect(copyDiscord)
...
Instance.new("UIStroke", RK).Color = Color3.fromRGB(255, 100, 100)
Instance.new("UIStroke", RK).Thickness = 2
...
-- ====================== MOTOR6D FIX (Anti Ragdoll Helper) ======================
local function fixMotor6D(char)
...
print("Msmsm Hub v8.0 - Part 1/4 Loaded ✅")
...
-- ====================== AUTO DUEL (Swapped Coordinates) ======================
-- Left Play uses R points (were Right Play before)
-- Right Play uses P points (were Left Play before)
local DC, DP, DR, DRP = nil, 1, false, 1

-- Right Play points (now used by Right Play)
local P1 = Vector3.new(-472.61, -6.81, 90.19)
local P2 = Vector3.new(-472.78, -6.81, 91.65)
local PE = Vector3.new(-486.38, -4.58, 95.27)

-- Left Play points (now used by Left Play)
local R1 = Vector3.new(-473.05, -6.81, 30.29)
local R2 = Vector3.new(-473.19, -6.81, 28.32)
local RE = Vector3.new(-485.97, -4.63, 25.18)
...
function startDuelReturn(returnDir)
    DR = true; DRP = 1
    if DC then DC:Disconnect() end
    local waypoints = returnDir == "right" and {P1, R1, R2} or {R1, P1, P2}
    DC = RunService.Heartbeat:Connect(function()
        if not Config.DuelEnabled then return end
        local r = getHRP()
        local h = getHum()
        if not r or not h then return end
        local target = waypoints[DRP]
        if not target then
            stopDuel()
        ...
        if (Vector3.new(target.X, r.Position.Y, target.Z) - r.Position).Magnitude < 2 then
            if DRP < #waypoints then
                DRP = DRP + 1
            else
                stopDuel()
                Config.DuelEnabled = false
...
function startDuelRight()
    stopDuel()
    DP = 1; DR = false
    local waypoints = {P1, P2, PE}
    DC = RunService.Heartbeat:Connect(function()
        if not Config.DuelEnabled then return end
        local r = getHRP()
        local h = getHum()
        if not r or not h then return end
        local target = waypoints[DP]
        if not target then
            startDuelReturn("right")
            return
        end
        local dir = (target - r.Position).Unit
        h:Move(dir)
...
-- ====================== AUTO GRAB (Blood Hub Enhanced) ======================
local GrabConn, IsGrabbing, GrabData, AnimalCache, PromptCache = nil, false, {}, {}, {}
...
function startBat()
    ...
    BCN = RunService.Heartbeat:Connect(function()
        ...
        if BAL then
            BAL.CFrame = CFrame.lookAt(mr.Position, Vector3.new(t.Position.X, mr.Position.Y, t.Position.Z))
        end
    end)
end
...
function stopBat()
    ...
    if BAT then BAT:Destroy(); BAT = nil end
    local h = getHum()
...
UserInputService.JumpRequest:Connect(function()
    ...
    local r = getHRP()
    if r then
        r.AssemblyLinearVelocity = Vector3.new(r.AssemblyLinearVelocity.X, Config.InventJumpPower, r.AssemblyLinearVelocity.Z)
    end
end)
...
function disableOptimizer()
    for p, v in pairs(XC) do
        if p then p.LocalTransparencyModifier = v end
    end
    XC = {}
end
...
function removeESP(pl)
    local c = pl.Character
    if c then
        local b = c:FindFirstChild("ESP_B")
...
local function CheckRagdollState(c)
    ...
    if hrp then
        local mo = hrp:FindFirstChildOfClass("Motor6D")
        if mo and not mo.Enabled then return true end
    end
    return false
end
...
function startBrainrotDefense()
    ...
    BrainrotConn = RunService.Heartbeat:Connect(function()
        ...
        if ragNow and not WasRagdolled then
            DoVortexTP()
        end
        WasRagdolled = ragNow
    end)
...
print("Msmsm Hub v8.0 - Part 2/4 Loaded ✅")
...
Instance.new("UIStroke", MM).Color = Color3.fromRGB(136, 0, 255)
Instance.new("UIStroke", MM).Thickness = 2
...
-- Brainrot Defense Side Selector
do
    ...
    Instance.new("UIStroke", sideBtn).Color = Color3.fromRGB(136, 0, 255)
...
Instance.new("UIStroke", MB).Color = Color3.fromRGB(136, 0, 255)
Instance.new("UIStroke", MB).Thickness = 2
...
print("Msmsm Hub v8.0 - Part 3/4 Loaded ✅")
...
print("Msmsm Hub v8.0 - Part 4/4 Loaded ✅")
print("All systems activated!")
print("Keybinds:")
print("  RCTRL = Menu    R = Right Play    L = Left Play    T = Duel Stop")
print("  G = Grab        B = Bat           P = Drop         F = Float")
print("  V = Spin        J = Invent Jump   U = TP Down      N = Unwalk")
print("  Y = Galaxy      X = Xray          M = Anti Fling   Z = Hitbox")
print("  O = FOV         K = ESP           H = Brainrot     ; = Medusa")
