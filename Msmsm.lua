...
local function saveConfig()
    ...
    if FloatBtns then for k, v in pairs(FloatBtns) do if v.btn then d["fb_"..k] = {x = v.btn.Position.X.Offset, y = v.btn.Position.Y.Offset} end end end
    pcall(function() writefile(CFG, HttpService:JSONEncode(d)) end)
end
local function loadConfig()
    if not isfile or not readfile then return end
    pcall(function() if isfile(CFG) then local d = HttpService:JSONDecode(readfile(CFG)); if d.Config then for k, v in pairs(d.Config) do if Config[k] ~= nil then Config[k] = v end end end end end)
end
loadConfig()

local Screen = Instance.new("ScreenGui", gethui and gethui() or CoreGui)
Screen.Name = "MsmsmHub_v8"; Screen.ResetOnSpawn = false; Screen.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
function notify(msg, dur)
    ...
    local f = Instance.new("Frame", g); f.Size = UDim2.new(0, 220, 0, 40); f.Position = UDim2.new(0.5, -110, 0, -50)
    ...
    local t = Instance.new("TextLabel", f); t.Size = UDim2.new(1, 0, 1, 0); t.BackgroundTransparency = 1; t.Text = msg
    t.TextColor3 = Color3.fromRGB(255, 255, 255); t.Font = Enum.Font.GothamBold; t.TextSize = 13; t.TextXAlignment = Enum.TextXAlignment.Center
...
local BTL = false
local LK = Instance.new("TextButton", Screen); LK.Size = UDim2.new(0, 60, 0, 32); LK.Position = UDim2.new(0.5, -66, 0, 42)
LK.BackgroundColor3 = Color3.fromRGB(12, 12, 12); LK.BorderSizePixel = 0; LK.Text = "🔒"
LK.TextColor3 = Color3.fromRGB(255, 255, 255); LK.Font = Enum.Font.GothamBold; LK.TextSize = 14; LK.AutoButtonColor = false; LK.ZIndex = 200
Instance.new("UICorner", LK).CornerRadius = UDim.new(0, 8)
local lS = Instance.new("UIStroke", LK); lS.Color = Color3.fromRGB(136, 0, 255); lS.Thickness = 2
LK.MouseButton1Click:Connect(function() BTL = not BTL; LK.Text = BTL and "🔐" or "🔒"; lS.Color = BTL and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(136, 0, 255) end)

local DPOS = {["RIGHT\nPLAY"]={90,60},["LEFT\nPLAY"]={160,60},["BAT"]={90,130},["DROP"]={160,130},["FLOAT"]={90,200},["DUEL\nSTOP"]={160,200},["TP\nDOWN"]={90,270}}
local function resetAllButtons() for n,p in pairs(DPOS) do if FloatBtns[n] then FloatBtns[n].btn.Position = UDim2.new(0,p[1],0,p[2]) end end; notify("✅ Buttons Reset!", 2) end
local RK = Instance.new("TextButton", Screen); RK.Size = UDim2.new(0,60,0,32); RK.Position = UDim2.new(0.5,6,0,42)
RK.BackgroundColor3 = Color3.fromRGB(12,12,12); RK.BorderSizePixel = 0; RK.Text = "🔄"
RK.TextColor3 = Color3.fromRGB(255,255,255); RK.Font = Enum.Font.GothamBold; RK.TextSize = 14; RK.AutoButtonColor = false; RK.ZIndex = 200
Instance.new("UICorner", RK).CornerRadius = UDim.new(0,8); Instance.new("UIStroke", RK).Color = Color3.fromRGB(255,100,100); Instance.new("UIStroke", RK).Thickness = 2
RK.MouseButton1Click:Connect(resetAllButtons)

local SBD = nil
LocalPlayer.CharacterAdded:Connect(function(c) task.wait(0.3); local hd = c:FindFirstChild("Head")
    if hd then if SBD and SBD.Parent then SBD:Destroy() end; SBD = Instance.new("BillboardGui", hd); SBD.Size = UDim2.new(0,100,0,18); SBD.StudsOffset = Vector3.new(0,2.8,0); SBD.AlwaysOnTop = true
        local lb = Instance.new("TextLabel", SBD); lb.Size = UDim2.new(1,0,1,0); lb.BackgroundTransparency = 1; lb.TextColor3 = Color3.fromRGB(255,255,255); lb.Font = Enum.Font.GothamBold; lb.TextSize = 13; lb.TextStrokeTransparency = 0.4 end
end)
RunService.RenderStepped:Connect(function() if SBD and SBD.Parent then local r = getHRP(); if r then local sp = math.floor(Vector3.new(r.AssemblyLinearVelocity.X,0,r.AssemblyLinearVelocity.Z).Magnitude+0.5); local lb = SBD:FindFirstChildOfClass("TextLabel"); if lb then lb.Text = sp end end end end)

FloatBtns = {}
function createFloatBtn(name, posX, posY, callback)
    local btn = Instance.new("TextButton", Screen); btn.Size = UDim2.new(0,64,0,64); btn.Position = UDim2.new(0,posX,0,posY)
    btn.BackgroundColor3 = Color3.fromRGB(12,12,12); btn.BorderSizePixel = 0; btn.Text = ""; btn.AutoButtonColor = false; btn.ZIndex = 20
    Instance.new("UICorner", btn).CornerRadius = UDim.new(0,14)
    local stk = Instance.new("UIStroke", btn); stk.Color = Color3.fromRGB(136,0,255); stk.Thickness = 2; stk.Transparency = 0.3
    local lbl = Instance.new("TextLabel", btn); lbl.Size = UDim2.new(1,0,1,0); lbl.BackgroundTransparency = 1; lbl.Text = name; lbl.TextColor3 = Color3.fromRGB(255,255,255); lbl.Font = Enum.Font.GothamBlack; lbl.TextSize = 10; lbl.TextWrapped = true
    local dot = Instance.new("Frame", btn); dot.Size = UDim2.new(0,8,0,8); dot.Position = UDim2.new(0.5,-4,1,-12); dot.BackgroundColor3 = Color3.fromRGB(60,60,60); dot.BorderSizePixel = 0; Instance.new("UICorner", dot).CornerRadius = UDim.new(1,0)
    local state = false
    btn.MouseButton1Click:Connect(function() state = not state; stk.Color = state and Color3.fromRGB(0,255,100) or Color3.fromRGB(136,0,255); stk.Transparency = state and 0 or 0.3; dot.BackgroundColor3 = state and Color3.fromRGB(0,255,100) or Color3.fromRGB(60,60,60); callback(state) end)
    local drag, ds, sp = false, nil, nil
    btn.InputBegan:Connect(function(input) if BTL then return end; if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then drag = true; ds = input.Position; sp = btn.Position end end)
    UserInputService.InputChanged:Connect(function(input) if drag and not BTL and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then local d = input.Position-ds; btn.Position = UDim2.new(sp.X.Scale, sp.X.Offset+d.X, sp.Y.Scale, sp.Y.Offset+d.Y) end end)
    ...
    FloatBtns[name] = {btn = btn, setState = function(s) state = s; stk.Color = s and Color3.fromRGB(0,255,100) or Color3.fromRGB(136,0,255); stk.Transparency = s and 0 or 0.3; dot.BackgroundColor3 = s and Color3.fromRGB(0,255,100) or Color3.fromRGB(60,60,60) end}
    return btn
end
...
function stopAntiRagdoll() if ARC then ARC:Disconnect(); ARC = nil end end
...
function stopSpin() if SB then SB:Destroy(); SB = nil end end
...
function stopFloat() if FC then FC:Disconnect(); FC = nil end end
...
function updateGalaxy()
    ...
    if not c then return end
    local m = 0
    for _, p in ipairs(c:GetDescendants()) do if p:IsA("BasePart") then m = m + p:GetMass() end end
...
function startDuelReturn(d)
    ...
    DC = RunService.Heartbeat:Connect(function()
        ...
        local t = w[DRP]; if not t then stopDuel(); return end
...
function startDuelRight()
    ...
    DC = RunService.Heartbeat:Connect(function()
        if not Config.DuelEnabled then return end
        local r = getHRP(); if not r then return end
        local t = w[DP]; if not t then startDuelReturn("right"); return end
        local flatTarget = Vector3.new(t.X, r.Position.Y, t.Z)
        local dir = (flatTarget - r.Position).Unit
...
function startDuelLeft()
    ...
    DC = RunService.Heartbeat:Connect(function()
        if not Config.DuelEnabled then return end
        local r = getHRP(); if not r then return end
        local t = w[DP]; if not t then startDuelReturn("left"); return end
        local flatTarget = Vector3.new(t.X, r.Position.Y, t.Z)
        local dir = (flatTarget - r.Position).Unit
        r.AssemblyLinearVelocity = Vector3.new(dir.X * Config.Speed, r.AssemblyLinearVelocity.Y, dir.Z * Config.Speed)
        if (flatTarget - r.Position).Magnitude < 2 then
...
-- ====================== AUTO GRAB (قوي - fireproximityprompt + getconnections) ======================
local GrabConn, IsGrabbing, GrabData, AnimalCache = nil, false, {}, {}
local GrabRadius, GrabDuration = Config.GrabRadius or 20, Config.GrabDuration or 0.12
...
local function ExecGrab(animal)
    ...
    -- طريقة getconnections مع Progress Bar
    local st = tick()
...
-- ====================== BAT AIMBOT ======================
local BCN, BAL, BATa = nil, nil, nil; local LE, LU = 0, 0
local function findBat()
    local c = getChar(); if not c then return nil end
    local bp = LocalPlayer:FindFirstChildOfClass("Backpack")
    for _, t in ipairs(c:GetChildren()) do if t:IsA("Tool") and t.Name:lower():find("bat") then return t end end
    if bp then for _, t in ipairs(bp:GetChildren()) do if t:IsA("Tool") and t.Name:lower():find("bat") then return t end end end
    return nil
end
local function nearestEnemy()
    local r = getHRP(); if not r then return nil, math.huge end
...
function startBat()
    ...
    BCN = RunService.Heartbeat:Connect(function()
        ...
        mr.CanCollide = false
        local t, dist = nearestEnemy(); if not t then return end
        mr.AssemblyLinearVelocity = (t.Position - mr.Position).Unit * Config.BatSpeed
        if dist <= Config.BatRange then
            if tick() - LE >= 0.3 then
                local b = findBat(); if b and b.Parent ~= c then mh:EquipTool(b) end; LE = tick()
            end
            if tick() - LU >= 0.3 then
                local b = findBat(); if b then b:Activate() end; LU = tick()
            end
        end
...
function startUnwalk()
    ...
    local c = getChar()
    if c then local a = c:FindFirstChild("Animate"); if a then SAV = a:Clone(); a:Destroy() end end
...
function stopUnwalk()
    ...
    local c = getChar(); if c and SAV then SAV:Clone().Parent = c; SAV = nil end
end
...
function expandPrompts()
    local p = workspace:FindFirstChild("Plots"); if not p then return end
    for _, o in ipairs(p:GetDescendants()) do
        if o:IsA("ProximityPrompt") then
            pcall(function() o.MaxActivationDistance = Config.HitboxRadius; o.RequiresLineOfSight = false end)
        end
    end
end
function restorePrompts()
    ...
    for _, o in ipairs(p:GetDescendants()) do
        if o:IsA("ProximityPrompt") then
            pcall(function() o.MaxActivationDistance = 10; o.RequiresLineOfSight = true end)
        end
    end
end
    ...
    pcall(function()
        for _, o in ipairs(workspace:GetDescendants()) do
            if o:IsA("BasePart") and o.Anchored and (o.Name:lower():find("base") or (o.Parent and o.Parent.Name:lower():find("base"))) then
                XC[o] = o.LocalTransparencyModifier; o.LocalTransparencyModifier = 0.88
            end
        end
    end)
end
...
function createESP(pl)
    ...
    b.Name = "ESP_B"; b.Adornee = hr; b.Size = Vector3.new(4, 6, 2)
    b.Color3 = Color3.fromRGB(136, 0, 255); b.Transparency = 0.5; b.AlwaysOnTop = true
end
...
function enableESP()
    ...
    Players.PlayerAdded:Connect(function(pl)
        pl.CharacterAdded:Connect(function() task.wait(0.3); if Config.EspEnabled then createESP(pl) end end)
    end)
end
function disableESP()
...
-- ====================== FOV ======================
function setFOV(v) pcall(function() Camera.FieldOfView = v and 100 or 70 end) end
...
function stopAntiFling() if AFC then AFC:Disconnect(); AFC = nil end end
...
function stopBrainrotDefense() if BrainrotConn then BrainrotConn:Disconnect(); BrainrotConn = nil end end
LocalPlayer.CharacterAdded:Connect(function() AlreadyTP = false; WasRagdolled = false end)
...
local function UseMedusaCounter()
    ...
    local char = getChar(); if not char then return end; MedusaDebounce = true
    local med = FindMedusaTool(); if not med then MedusaDebounce = false; return end
    if med.Parent ~= char then local hum = getHum(); if hum then hum:EquipTool(med) end end
...
local function SetupMedusaCounter(char)
    ...
    local function onAnchorChanged(part) return part:GetPropertyChangedSignal("Anchored"):Connect(function() if Config.MedusaCounterEnabled and part.Anchored and part.Transparency == 1 then UseMedusaCounter() end end) end
...
MM.Size = UDim2.new(0, 230, 0, 370); MM.Position = UDim2.new(0.5, -115, 0.5, -185)
MM.BackgroundColor3 = Color3.fromRGB(10, 10, 10); MM.BorderSizePixel = 0; MM.Visible = false; MM.ZIndex = 30
Instance.new("UICorner", MM).CornerRadius = UDim.new(0, 14)
Instance.new("UIStroke", MM).Color = Color3.fromRGB(136, 0, 255); Instance.new("UIStroke", MM).Thickness = 2

local mT = Instance.new("TextLabel", MM); mT.Size = UDim2.new(1, 0, 0, 30); mT.BackgroundTransparency = 1
mT.Text = "Msmsm Hub v8.0"; mT.TextColor3 = Color3.fromRGB(255, 255, 255); mT.Font = Enum.Font.GothamBlack
mT.TextSize = 14; mT.TextXAlignment = Enum.TextXAlignment.Center; mT.Position = UDim2.new(0, 0, 0, 6)

local mC = Instance.new("TextButton", MM); mC.Size = UDim2.new(0, 26, 0, 26); mC.Position = UDim2.new(1, -32, 0, 6)
mC.BackgroundColor3 = Color3.fromRGB(25, 25, 25); mC.BorderSizePixel = 0; mC.Text = "X"
mC.TextColor3 = Color3.fromRGB(255, 255, 255); mC.Font = Enum.Font.GothamBold; mC.TextSize = 12
Instance.new("UICorner", mC).CornerRadius = UDim.new(0, 6); mC.MouseButton1Click:Connect(function() MM.Visible = false end)

local mDr = false; local mDS, mSP
MM.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then mDr = true; mDS = i.Position; mSP = MM.Position end end)
UserInputService.InputChanged:Connect(function(i) if mDr and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then local d = i.Position-mDS; MM.Position = UDim2.new(mSP.X.Scale, mSP.X.Offset+d.X, mSP.Y.Scale, mSP.Y.Offset+d.Y) end end)
UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then mDr = false end end)

local TB = {}; local TC = {}; local TN = {"Settings", "Combat", "Control", "Visual"}
local TBa = Instance.new("Frame", MM); TBa.Size = UDim2.new(1, 0, 0, 26); TBa.Position = UDim2.new(0, 0, 0, 36); TBa.BackgroundTransparency = 1; TBa.ZIndex = 31
for i, n in ipairs(TN) do
    local t = Instance.new("TextButton", TBa); t.Size = UDim2.new(0.25, -4, 1, 0); t.Position = UDim2.new((i-1)*0.25, 2, 0, 0)
    t.BackgroundColor3 = i == 1 and Color3.fromRGB(136, 0, 255) or Color3.fromRGB(25, 25, 25); t.BorderSizePixel = 0; t.Text = n
    t.TextColor3 = Color3.fromRGB(255, 255, 255); t.Font = Enum.Font.GothamBold; t.TextSize = 9; t.AutoButtonColor = false; t.ZIndex = 32
    Instance.new("UICorner", t).CornerRadius = UDim.new(0, 5)
    local c = Instance.new("ScrollingFrame", MM); c.Size = UDim2.new(1, -8, 1, -68); c.Position = UDim2.new(0, 4, 0, 64)
    c.BackgroundTransparency = 1; c.BorderSizePixel = 0; c.ScrollBarThickness = 3; c.ScrollBarImageColor3 = Color3.fromRGB(136, 0, 255)
    c.Visible = (i == 1); c.ZIndex = 31; c.CanvasSize = UDim2.new(0, 0, 0, 800); Instance.new("UIListLayout", c).Padding = UDim.new(0, 5)
    TB[i] = t; TC[i] = c
    t.MouseButton1Click:Connect(function() for j, tb in ipairs(TB) do tb.BackgroundColor3 = j == i and Color3.fromRGB(136, 0, 255) or Color3.fromRGB(25, 25, 25) end; for j, cc in ipairs(TC) do cc.Visible = (j == i) end end)
end

local function CSL(p, n, min, max, def, cb)
    local ct = Instance.new("Frame", p); ct.Size = UDim2.new(1, 0, 0, 42); ct.BackgroundTransparency = 1; ct.ZIndex = 32
    local lb = Instance.new("TextLabel", ct); lb.Size = UDim2.new(1, -50, 0, 14); lb.BackgroundTransparency = 1; lb.Text = n
    lb.TextColor3 = Color3.fromRGB(200, 200, 200); lb.Font = Enum.Font.GothamBold; lb.TextSize = 10; lb.TextXAlignment = Enum.TextXAlignment.Left; lb.ZIndex = 33
    local bx = Instance.new("TextBox", ct); bx.Size = UDim2.new(0, 42, 0, 16); bx.Position = UDim2.new(1, -42, 0, 0)
    bx.BackgroundColor3 = Color3.fromRGB(25, 25, 25); bx.BorderSizePixel = 0; bx.Text = tostring(def); bx.TextColor3 = Color3.fromRGB(255, 255, 255)
    bx.Font = Enum.Font.GothamBold; bx.TextSize = 9; bx.TextXAlignment = Enum.TextXAlignment.Center; bx.ClearTextOnFocus = false; bx.ZIndex = 33
    Instance.new("UICorner", bx).CornerRadius = UDim.new(0, 4)
    local bg = Instance.new("Frame", ct); bg.Size = UDim2.new(1, 0, 0, 6); bg.Position = UDim2.new(0, 0, 0, 22)
    bg.BackgroundColor3 = Color3.fromRGB(30, 30, 30); bg.BorderSizePixel = 0; bg.ZIndex = 33; Instance.new("UICorner", bg).CornerRadius = UDim.new(1, 0)
    local fl = Instance.new("Frame", bg); fl.Size = UDim2.new((def-min)/(max-min), 0, 1, 0); fl.BackgroundColor3 = Color3.fromRGB(136, 0, 255)
    fl.BorderSizePixel = 0; fl.ZIndex = 34; Instance.new("UICorner", fl).CornerRadius = UDim.new(1, 0)
    local sb = Instance.new("TextButton", bg); sb.Size = UDim2.new(1, 0, 2, 0); sb.BackgroundTransparency = 1; sb.Text = ""; sb.ZIndex = 35
    local function up(v) local c = math.clamp(math.floor(v), min, max); fl.Size = UDim2.new((c-min)/(max-min), 0, 1, 0); bx.Text = tostring(c); cb(c) end
    bx.FocusLost:Connect(function() local n = tonumber(bx.Text); if n then up(n) else bx.Text = tostring(def) end end)
    local sd = false; sb.MouseButton1Down:Connect(function() sd = true end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then sd = false end end)
    UserInputService.InputChanged:Connect(function(i) if sd and i.UserInputType == Enum.UserInputType.MouseMovement then local pct = math.clamp((i.Position.X-bg.AbsolutePosition.X)/bg.AbsoluteSize.X,0,1); up(min+(max-min)*pct) end end)
end

MenuToggles = {}
local function CMT(p, n, def, cb)
    local ct = Instance.new("Frame", p); ct.Size = UDim2.new(1, 0, 0, 30); ct.BackgroundTransparency = 1; ct.ZIndex = 32
    local lb = Instance.new("TextLabel", ct); lb.Size = UDim2.new(1, -48, 1, 0); lb.Position = UDim2.new(0, 6, 0, 0); lb.BackgroundTransparency = 1; lb.Text = n
    lb.TextColor3 = Color3.fromRGB(255, 255, 255); lb.Font = Enum.Font.GothamBold; lb.TextSize = 11; lb.TextXAlignment = Enum.TextXAlignment.Left; lb.ZIndex = 33
    local tk = Instance.new("Frame", ct); tk.Size = UDim2.new(0, 36, 0, 16); tk.Position = UDim2.new(1, -42, 0.5, -8)
    tk.BackgroundColor3 = def and Color3.fromRGB(136, 0, 255) or Color3.fromRGB(40, 40, 48); tk.BorderSizePixel = 0; tk.ZIndex = 33; Instance.new("UICorner", tk).CornerRadius = UDim.new(1, 0)
    local dt = Instance.new("Frame", tk); dt.Size = UDim2.new(0, 12, 0, 12); dt.Position = def and UDim2.new(1,-13,0.1,0) or UDim2.new(0,2,0.1,0)
    dt.BackgroundColor3 = Color3.fromRGB(255, 255, 255); dt.BorderSizePixel = 0; dt.ZIndex = 34; Instance.new("UICorner", dt).CornerRadius = UDim.new(1, 0)
    local st = def
    local b = Instance.new("TextButton", ct); b.Size = UDim2.new(1, 0, 1, 0); b.BackgroundTransparency = 1; b.Text = ""; b.ZIndex = 35
    b.MouseButton1Click:Connect(function() st = not st; tk.BackgroundColor3 = st and Color3.fromRGB(136,0,255) or Color3.fromRGB(40,40,48); dt.Position = st and UDim2.new(1,-13,0.1,0) or UDim2.new(0,2,0.1,0); cb(st) end)
    local mt = {setState = function(s) st = s; tk.BackgroundColor3 = s and Color3.fromRGB(136,0,255) or Color3.fromRGB(40,40,48); dt.Position = s and UDim2.new(1,-13,0.1,0) or UDim2.new(0,2,0.1,0) end}
    MenuToggles[n] = mt; return mt
end
...
CSL(TC[1], "Spin Spd", 10, 500, Config.SpinSpeed, function(v) Config.SpinSpeed = v; if SB then SB.AngularVelocity = Vector3.new(0, v, 0) end end)
CSL(TC[1], "Float Pwr", 5, 50, Config.FloatPower, function(v) Config.FloatPower = v end)
CSL(TC[1], "Gravity %", 10, 95, Config.GalaxyGravity, function(v) Config.GalaxyGravity = v; if Config.GalaxyEnabled then stopGalaxy(); startGalaxy() end end)
CSL(TC[1], "Jump Pwr", 10, 200, Config.InventJumpPower, function(v) Config.InventJumpPower = v end)
CSL(TC[1], "Grab Rad", 5, 200, Config.GrabRadius, function(v) Config.GrabRadius = v; GrabRadius = v end)
CSL(TC[1], "Bat Spd", 10, 200, Config.BatSpeed, function(v) Config.BatSpeed = v end)
local svB = Instance.new("TextButton", TC[1]); svB.Size = UDim2.new(1, 0, 0, 36); svB.BackgroundColor3 = Color3.fromRGB(136, 0, 255); svB.BorderSizePixel = 0
svB.Text = "SAVE"; svB.TextColor3 = Color3.fromRGB(255, 255, 255); svB.Font = Enum.Font.GothamBlack; svB.TextSize = 16; svB.ZIndex = 32
Instance.new("UICorner", svB).CornerRadius = UDim.new(0, 8)
svB.MouseButton1Click:Connect(function() saveConfig(); svB.Text = "✅ SAVED!"; task.delay(1.5, function() svB.Text = "SAVE" end) end)

-- Tab 2: Combat
CMT(TC[2], "RIGHT PLAY", false, function(s) Config.DuelEnabled = s; if s then startDuelRight() else stopDuel() end; if FloatBtns["RIGHT\nPLAY"] then FloatBtns["RIGHT\nPLAY"].setState(s) end end)
CMT(TC[2], "LEFT PLAY", false, function(s) Config.DuelEnabled = s; if s then startDuelLeft() else stopDuel() end; if FloatBtns["LEFT\nPLAY"] then FloatBtns["LEFT\nPLAY"].setState(s) end end)
CMT(TC[2], "GRAB", false, function(s) Config.AutoGrabEnabled = s; if s then startGrab() else stopGrab() end end)
CMT(TC[2], "BAT", false, function(s) Config.BatAimbotEnabled = s; if s then startBat() else stopBat() end; if FloatBtns["BAT"] then FloatBtns["BAT"].setState(s) end end)
CMT(TC[2], "DROP", false, function(s) if s then task.spawn(dropBrainrot) end end)
CMT(TC[2], "DUEL STOP", false, function(s) Config.DuelEnabled = false; stopDuel(); if FloatBtns["RIGHT\nPLAY"] then FloatBtns["RIGHT\nPLAY"].setState(false) end; if FloatBtns["LEFT\nPLAY"] then FloatBtns["LEFT\nPLAY"].setState(false) end end)
CMT(TC[2], "SPIN", false, function(s) Config.SpinEnabled = s; if s then startSpin() else stopSpin() end end)
CMT(TC[2], "INVENT JUMP", false, function(s) Config.InventJumpEnabled = s end)
CMT(TC[2], "TP DOWN", false, function(s) if s then tpDown() end end)
CMT(TC[2], "UNWALK", false, function(s) Config.UnwalkEnabled = s; if s then startUnwalk() else stopUnwalk() end end)

-- Tab 3: Control
CMT(TC[3], "Galaxy", false, function(s) Config.GalaxyEnabled = s; if s then startGalaxy() else stopGalaxy() end end)
CMT(TC[3], "Xray", false, function(s) Config.OptimizerEnabled = s; if s then enableOptimizer() else disableOptimizer() end end)
CMT(TC[3], "Anti Fling", false, function(s) Config.AntiFlingEnabled = s; if s then startAntiFling() else stopAntiFling() end end)
CMT(TC[3], "Brainrot Def", false, function(s) Config.BrainrotDefenseEnabled = s; if s then startBrainrotDefense() else stopBrainrotDefense() end end)
do
    local ct = Instance.new("Frame", TC[3]); ct.Size = UDim2.new(1, 0, 0, 30); ct.BackgroundTransparency = 1; ct.ZIndex = 32
    local lb = Instance.new("TextLabel", ct); lb.Size = UDim2.new(0, 80, 1, 0); lb.Position = UDim2.new(0, 6, 0, 0); lb.BackgroundTransparency = 1; lb.Text = "Defense Side"
    lb.TextColor3 = Color3.fromRGB(255, 255, 255); lb.Font = Enum.Font.GothamBold; lb.TextSize = 11; lb.TextXAlignment = Enum.TextXAlignment.Left; lb.ZIndex = 33
    local sideBtn = Instance.new("TextButton", ct); sideBtn.Size = UDim2.new(0, 42, 0, 18); sideBtn.Position = UDim2.new(1, -48, 0.5, -9)
    sideBtn.BackgroundColor3 = Color3.fromRGB(30, 10, 60); sideBtn.BorderSizePixel = 0
    sideBtn.Text = Config.BrainrotDefenseSide == "left" and "LEFT" or "RIGHT"; sideBtn.TextColor3 = Color3.fromRGB(200, 140, 255)
    sideBtn.Font = Enum.Font.GothamBold; sideBtn.TextSize = 8; sideBtn.ZIndex = 33
    Instance.new("UICorner", sideBtn).CornerRadius = UDim.new(0, 4); Instance.new("UIStroke", sideBtn).Color = Color3.fromRGB(136, 0, 255)
    sideBtn.MouseButton1Click:Connect(function() Config.BrainrotDefenseSide = Config.BrainrotDefenseSide == "left" and "right" or "left"; sideBtn.Text = Config.BrainrotDefenseSide == "left" and "LEFT" or "RIGHT"; saveConfig() end)
end
CMT(TC[3], "Medusa Counter", false, function(s) Config.MedusaCounterEnabled = s; if s and LocalPlayer.Character then SetupMedusaCounter(LocalPlayer.Character) else StopMedusaCounter() end end)

-- Tab 4: Visual
CMT(TC[4], "Hitbox Exp", false, function(s) Config.HitboxEnabled = s; if s then expandPrompts() else restorePrompts() end end)
CMT(TC[4], "FOV", false, function(s) Config.FovEnabled = s; setFOV(s) end)
CMT(TC[4], "ESP", false, function(s) if s then enableESP() else disableESP() end end)

-- Floating Buttons
createFloatBtn("RIGHT\nPLAY", 90, 60, function(s) Config.DuelEnabled = s; if s then startDuelRight() else stopDuel() end; if MenuToggles["RIGHT PLAY"] then MenuToggles["RIGHT PLAY"].setState(s) end end)
createFloatBtn("LEFT\nPLAY", 160, 60, function(s) Config.DuelEnabled = s; if s then startDuelLeft() else stopDuel() end; if MenuToggles["LEFT PLAY"] then MenuToggles["LEFT PLAY"].setState(s) end end)
createFloatBtn("BAT", 90, 130, function(s) Config.BatAimbotEnabled = s; if s then startBat() else stopBat() end; if MenuToggles["BAT"] then MenuToggles["BAT"].setState(s) end end)
createFloatBtn("DROP", 160, 130, function(s) if s then task.spawn(dropBrainrot) end end)
createFloatBtn("FLOAT", 90, 200, function(s) Config.FloatEnabled = s; if s then startFloat() else stopFloat() end end)
createFloatBtn("DUEL\nSTOP", 160, 200, function(s) Config.DuelEnabled = false; stopDuel(); if FloatBtns["RIGHT\nPLAY"] then FloatBtns["RIGHT\nPLAY"].setState(false) end; if FloatBtns["LEFT\nPLAY"] then FloatBtns["LEFT\nPLAY"].setState(false) end; if MenuToggles["RIGHT PLAY"] then MenuToggles["RIGHT PLAY"].setState(false) end; if MenuToggles["LEFT PLAY"] then MenuToggles["LEFT PLAY"].setState(false) end end)
createFloatBtn("TP\nDOWN", 90, 270, function(s) if s then tpDown() end end)

local MB = Instance.new("TextButton", Screen); MB.Size = UDim2.new(0, 44, 0, 44); MB.Position = UDim2.new(0.5, -22, 0, 80)
MB.BackgroundColor3 = Color3.fromRGB(12, 12, 12); MB.BorderSizePixel = 0; MB.Text = "M"
MB.TextColor3 = Color3.fromRGB(136, 0, 255); MB.Font = Enum.Font.GothamBlack; MB.TextSize = 18; MB.ZIndex = 100
Instance.new("UICorner", MB).CornerRadius = UDim.new(0, 12); Instance.new("UIStroke", MB).Color = Color3.fromRGB(136, 0, 255); Instance.new("UIStroke", MB).Thickness = 2
MB.MouseButton1Click:Connect(function() MM.Visible = not MM.Visible end)

if isfile and isfile(CFG) then pcall(function() local d = HttpService:JSONDecode(readfile(CFG)); for k, v in pairs(d) do if k:match("^fb_") and FloatBtns then local name = k:gsub("fb_", ""); if FloatBtns[name] and FloatBtns[name].btn then FloatBtns[name].btn.Position = UDim2.new(0, v.x, 0, v.y) end end end end) end
...
UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if input.KeyCode == Enum.KeyCode.P then task.spawn(dropBrainrot); return end
    ...
    if input.KeyCode == Enum.KeyCode.U then tpDown(); return end
    ...
    if input.KeyCode == Enum.KeyCode.Semicolon then
        ...
        if Config.MedusaCounterEnabled and LocalPlayer.Character then SetupMedusaCounter(LocalPlayer.Character) else StopMedusaCounter() end
...
-- ====================== CLEANUP ======================
LocalPlayer.OnTeleport:Connect(function() saveConfig() end)
...
print("  ✅ Auto Grab: fireproximityprompt + getconnections")
print(string.rep("=", 55))
print("  KEYBINDS:")
